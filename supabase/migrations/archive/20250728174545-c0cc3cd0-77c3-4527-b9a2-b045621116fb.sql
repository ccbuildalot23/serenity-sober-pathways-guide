...
-- Create RLS policies for data retention (using provider role instead of admin)
CREATE POLICY "Providers can manage retention policies" ON public.data_retention_policies
...
CREATE POLICY "Providers can manage backup verification" ON public.backup_verification_logs
FOR ALL USING (EXISTS (
  SELECT 1 FROM public.user_roles 
  WHERE user_id = auth.uid() AND role = 'provider'
));

CREATE POLICY "Providers can manage recovery drills" ON public.recovery_drill_schedules
FOR ALL USING (EXISTS (
  SELECT 1 FROM public.user_roles 
  WHERE user_id = auth.uid() AND role = 'provider'
));
...
CREATE POLICY "Providers can manage incidents" ON public.incident_responses
FOR ALL USING (EXISTS (
  SELECT 1 FROM public.user_roles 
  WHERE user_id = auth.uid() AND role = 'provider'
));

CREATE POLICY "Providers can manage regulatory notifications" ON public.regulatory_notifications
FOR ALL USING (EXISTS (
  SELECT 1 FROM public.user_roles 
  WHERE user_id = auth.uid() AND role = 'provider'
));
...
CREATE POLICY "Providers can manage compliance requirements" ON public.compliance_requirements
FOR ALL USING (EXISTS (
  SELECT 1 FROM public.user_roles 
  WHERE user_id = auth.uid() AND role = 'provider'
));

CREATE POLICY "Providers can view audit trails" ON public.compliance_audit_trails
